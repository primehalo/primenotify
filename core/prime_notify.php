...
* Include only once.
*/
if (!class_exists('primehalo\\primenotify\\core\\prime_notify'))
{
	/**
	* Class declaration
	*/
	class prime_notify
	{
		...
		*/
		...
		private $keep_users	= array();	// List of user IDs that have the always_send option enabled
		private $paths		= array();	// List of template directories that have been checked
		private $files		= array();	// List of template files that have been checked
		...
		*/
		protected function __construct()
		{
			...
			$this->bbcode_utils	= $phpbb_container->get('text_formatter.utils');
			$this->user_loader	= $phpbb_container->get('user_loader');
		...
		*/
		private function format_for_email($text, $keep_bbcodes = true)
		{
			...
			if (!$keep_bbcodes)
			{
				...
				$text = $this->bbcode_utils->remove_bbcode($text, 'spoiler', 2);
				$text = $this->bbcode_utils->remove_bbcode($text, 'quote', 2);
				$text = $this->bbcode_utils->remove_bbcode($text, 'code', 2);
			}

			// Return the text from its XML form to its original plain text form
			$text = $this->bbcode_utils->unparse($text);
